...
function FormComponent(props) {
  ...
  useEffect(() => {
    ...
    if (token !== '') {
      ...
      const processFromData = async () => {
        ...
        formvalues['phone'] = formvalues.telephone
        formvalues['address'] = address;
        formvalues['type'] = formvalues.property_type === "Sell" ? 'vendor' : "landlord";
        ...
        postFormData(formSubmitData).then(async apiRes => {
          ...
          // lets send mail
          let url = '/api/gatsby-theme-starberry-mira/functions';
  ...
  const handleSubmit = event => {
    ...
    if (form.checkValidity() === false) {
      ...
      const moveDateElement = document.getElementsByClassName('date-picker')
      const moveTimeElement = document.getElementsByClassName('time-picker')

      if (moveDateElement) {
        moveDateElement[0]?.classList?.add('move-left')
      }
      if (moveTimeElement) {
        moveTimeElement[0]?.classList?.add('move-left')
      }
      setTimeout(() => {
  ...
  return (
    <div className={props.classname}>
      ...
      <Form className={`form stb-form ${props.formtagclassname}`} id="contact-form" name={fields[0].formname} action="/thank-you/" method="post" noValidate validated={validated} onSubmit={handleSubmit}>
        ...
        {showerror && props.submit_tag !== "arrow" && <div className="alert alert-danger">
          {fields[0].error_text}
        </div>}
        ...
        <div className={props.submit_tag !== "arrow" ? "row" : ""}>
          {fields.map((field, index) => {
            ...
            if ("button" === field.element && props.submit_tag !== "arrow") {
              return (
                <ButtonField
                  name={field.name}
                  fieldClass={field.class}
...
const DefaultForm = (props) => {
  ...
  return (
    <FormComponent fields={props.fields} classname={props.classname} to_email_id={props.to_email_id} title={props.title} formtagclassname={props.formtagclassname} prop_url={`${props?.prop_url}`} prop_address={`${props?.prop_address}`} prop_img_url={`${props?.prop_img_url}`}
      email_template_type={`${props?.email_template_type}`} siteData={siteData} submit_tag={props.submit} />
  )
}
